...
- 現行の Kotlin runtime は repository root の Gradle project に統合済みだが、導入初期は独立した検証実装から始まっている
...
- 初期評価記録: [nsstatusitem-prototype.md](./nsstatusitem-prototype.md)
...
初期検証実装の zero-delay timer ベース更新がこれを再現できない場合、完全置換は停止する。
...
- 初期検証実装に散在している menu/state/settings 反映を production 向け state store と controller に分離する
- Kotlin 側を唯一の mutation 境界にする

主な対象:

- `src/macosArm64Main/.../Main.kt`
- `src/macosArm64Main/.../PersistedSettings.kt`
...
- `src/macosArm64Main/.../Main.kt`
- 当時の Swift `StatusBarController` を比較基準として参照
...
- 検証実装専用キーを廃止し、本番 UserDefaults キーへ統一する
- 既存の shared settings logic を app runtime 側へ統合する

主な対象:

- `src/commonMain/.../SettingsStoreLogic.kt`
- `src/macosMain/.../NSUserDefaultsKeyValueStore.kt`
- `src/macosArm64Main/.../PersistedSettings.kt`
...
- Kotlin runtime の auto appearance 切替実装
- Kotlin 側 tests
...
- Kotlin runtime の manual brightness 監視実装
- Kotlin 側 tests
...
- Kotlin/AppKit settings window 実装
- Kotlin 側 LaunchAgent 管理実装
