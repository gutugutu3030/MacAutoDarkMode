...
- プロトタイプ本体: [`../../prototypes/kmp-menubar-poc/README.md`](../../prototypes/kmp-menubar-poc/README.md)
- 実行コード: [`../../prototypes/kmp-menubar-poc/src/macosArm64Main/kotlin/com/github/gutugutu3030/autodarkmode/prototype/Main.kt`](../../prototypes/kmp-menubar-poc/src/macosArm64Main/kotlin/com/github/gutugutu3030/autodarkmode/prototype/Main.kt)

このプロトタイプは production の Swift Package とは独立させる。
理由は、失敗した場合でも既存アプリのビルド・配布・レビュー動線に影響を与えないようにするためである。
...
- 生成物 `build/bin/macosArm64/debugExecutable/kmp-menubar-poc.kexe` が起動する
- 起動後、`pgrep -fl 'kmp-menubar-poc\\.kexe|kmp-menubar-poc'` でプロセス常駐を確認できる

今回は headless な確認に留めており、**メニューバー上の見た目と操作感は手動確認が必要** である。

## 第 2 段検証結果: 動的メニュー更新

次段階として、現行 [StatusBarController](../../Sources/autoDarkMode/UI/StatusBarController.swift) が持つ更新責務の一部を Kotlin 側で再現した。
...
さらに、現行 [BrightnessKeyMonitor](../../Sources/autoDarkMode/Application/BrightnessKeyMonitor.swift) と [AutoSwitchEngine](../../Sources/autoDarkMode/Application/AutoSwitchEngine.swift) に相当する 2 系統の入力を、Kotlin 側プロトタイプへ別々に流し込んだ。
