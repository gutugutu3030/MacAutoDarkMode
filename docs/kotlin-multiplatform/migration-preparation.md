...
これらは現行 [StatusBarController](../../Sources/autoDarkMode/UI/StatusBarController.swift) の `updatePresentation()` に集まっているため、shared 化の候補として最も切り出しやすい。
...
1. `kmp/` サブプロジェクトを production とは独立に追加する
...
現時点の prototype は、ここでいう「設定変更が再起動なしでメニューバー表示へ伝播すること」を、`NSUserDefaultsDidChangeNotification` を経由する 1 本の更新経路として追加検証する段階にある。
...
- `kmp/` は Swift Package から独立して失敗できる
